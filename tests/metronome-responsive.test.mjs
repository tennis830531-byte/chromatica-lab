...
function accentLayout(viewport, signature) {
...
test("accent controls are a bounded responsive grid instead of a horizontal flex row", () => {
  const rule = css.match(/\.metronome-accents\s*\{([^}]*)\}/)?.[1] || "";
  assert.match(rule, /display:\s*grid/);
  assert.match(rule, /repeat\(var\(--metronome-accent-columns, 4\), minmax\(0, 1fr\)\)/);
  assert.match(rule, /width:\s*100%/);
  assert.match(rule, /min-width:\s*0/);
  assert.doesNotMatch(rule, /overflow-x|flex/);
});

test("accent buttons fill their grid cell without imposing intrinsic width", () => {
  const rule = css.match(/\.metronome-accents button\s*\{([^}]*)\}/)?.[1] || "";
  assert.match(rule, /width:\s*100%/);
...
test("rendering caps accent columns at four", () => {
  assert.match(ui, /--metronome-accent-columns[\s\S]*Math\.min\(4, Math\.max\(1, settings\.accents\.length\)\)/);
});

test("status, choice, BPM and trainer grids use zero-minimum tracks", () => {
  for (const selector of ["metronome-status-row", "metronome-bpm-controls", "metronome-choice-grid", "subdivision-grid", "metronome-trainer-grid"]) {
...
for (const viewport of viewports) {
  for (const signature of signatures) {
    test(`${viewport}px ${signature} accent grid remains within its content width`, () => {
      const layout = accentLayout(viewport, signature);
...
test("four-column wrapping makes beats 5 through 12 visible on later rows", () => {
  assert.deepEqual(accentLayout(320, "5/4"), { numerator: 5, columns: 4, rows: 2, pageWidth: 296, contentWidth: 256, cellWidth: 58 });
  assert.equal(accentLayout(320, "9/8").rows, 3);
  assert.equal(accentLayout(320, "12/8").rows, 3);
});
