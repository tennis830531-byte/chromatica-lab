...
test("old backend fallback remains safe and cannot spin forever", () => {
  ...
  assert.match(runtime, /const defeated = zeroHp \|\| \(resultVisible[\s\S]*status === "defeated"/);
  assert.match(runtime, /label\.textContent = `\$\{presentation\.name\}出沒了！`/);
  assert.match(runtime, /label\.textContent = `\$\{presentation\.name\}被擊倒了！`/);
  assert.match(runtime, /if \(defeated\) label\.textContent = `\$\{presentation\.name\}被擊倒了！`;[\s\S]*state\.status === "unavailable"/);
  assert.match(runtime, /label\.textContent = "討伐失敗！"/);
  assert.match(runtime, /button\.classList\.toggle\("is-dormant", !active && !defeated\)/);
  assert.match(runtime, /const zeroHp = state\.event && Number\(state\.event\.remaining_hp\) <= 0/);
  assert.match(runtime, /resultStatuses\.has\(status\) \|\| zeroHp/);
