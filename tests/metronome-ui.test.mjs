...
test("metronome stage uses the dedicated navy and gold supplied background", () => { const css=fs.readFileSync(path.join(root,"styles.css"),"utf8"); assert.match(css, /\.metronome-stage\s*\{[^}]*metronome-stage-blue\.png/); assert.match(css, /\.metronome-stage::before\s*\{[^}]*rgba\(242, 199, 106/); assert.doesNotMatch(css, /\.metronome-stage::before\s*\{[^}]*tuner-card-bg\.png/); });
test("metronome and tuner primary cards share exact sizing tokens", () => { assert.match(styles, /\.tuner-page-card,\s*\.metronome-stage\s*\{[^}]*width:\s*100%[^}]*height:\s*clamp\(420px, calc\(100vw - 42px\), 590px\)[^}]*padding:\s*18px[^}]*border-radius:\s*28px/); });
test("start is beside Tap Tempo and the obsolete two-tap hint is not visible", () => { assert.match(html, /class="metronome-tap-row"[\s\S]*id="metronomeTap"[\s\S]*id="metronomeToggle"/); assert.doesNotMatch(html, />至少點兩下</); });
