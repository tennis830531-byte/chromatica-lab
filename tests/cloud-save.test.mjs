    ...
    return {
      select: () => ({
        eq: (_column, value) => {
          userId = value;
          return {
            maybeSingle: async () => ({ data: this.rows.get(userId) || null, error: this.readError }),
          };
        },
      }),
    };
...
test("an unknown local and remote divergence becomes a conflict without overwriting local data", async () => {
  const storage = new MemoryStorage();
  ...
  assert.equal(storage.getItem("chromatica.waterDrops"), "5");
  assert.equal(readCloudSyncMeta("user-a", storage).conflict, true);

  const resolution = await service.resolveConflict("remote");
...
test("choosing the local conflict copy uploads with the stored remote revision", async () => {
  const storage = new MemoryStorage();
  const local = makeSnapshot("user-a", 19);
  const remote = makeSnapshot("user-a", 11);
  storage.setItem(ACTIVE_ACCOUNT_KEY, "user-a");
  storeAccountSnapshot("user-a", local, storage);
  Object.entries(local.data).forEach(([key, value]) => storage.setItem(key, value));
  const client = new MockSupabaseClient(new Map([["user-a", {
  ...
  const service = createCloudSaveService({ client, storage });
  assert.equal((await service.reconcileStartup("user-a", local)).kind, "conflict");

  const resolution = await service.resolveConflict("local");
  assert.equal(resolution.ok, true);
  assert.equal(client.rpcCalls.at(-1).p_expected_revision, 6);
  assert.equal(client.rows.get("user-a").snapshot.data["chromatica.waterDrops"], "19");
});
