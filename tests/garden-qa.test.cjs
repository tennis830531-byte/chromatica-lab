...
test("QA garden watering remains infinite while isolated skill water covers all six skills", () => { assert.match(source, /qa\$\("#gardenQaWater"\)[\s\S]*water\.textContent = "∞"/); assert.match(source, /QA_SKILL_TEST_WATER = 600/); assert.match(source, /qaWaterDrops: QA_SKILL_TEST_WATER/); assert.match(source, /worldBossSkillUnlocks: \[\]/); assert.doesNotMatch(source, /setWaterDrops|chromatica\.waterDrops|localStorage/); });
test("QA rename restores stage names when an official species name is entered", () => { assert.match(source, /function isDefaultSpeciesName/); assert.match(source, /spirit\.customName = !isDefaultSpeciesName\(spirit\.species, name\)/); });
